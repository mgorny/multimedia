...
src_prepare() {
	...
	if [[ ${PV%_p*} != ${PV} ]]; then
		sed -i -e "s/UNKNOWN/DATE-${PV#*_pre}/" version.sh || die
	fi
	...
	epatch "${FILESDIR}"/ffmpeg-ffpreset.patch
}
